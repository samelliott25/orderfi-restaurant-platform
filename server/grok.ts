...
// Generate novel moving background using Grok-4
export async function generateNovelMovingBackground(): Promise<string> {
  try {
    const response = await openai.chat.completions.create({
      model: "grok-2-1212",
      messages: [{
...
Generate complete React component code with all CSS animations, SVG filters, and TypeScript interfaces. Focus on creating a premium, restaurant-grade visual experience that enhances the OrderFi brand without distracting from functionality.`
    ...
    console.error('Error generating novel background:', error);
    throw new Error('Failed to generate novel moving background');
  }
}