...
import { RealTimeStatusIndicator, RealTimeEventsMonitor } from './RealTimeSync';
import { ContextStatusIndicator, ContextAwareAlerts, ContextAwareQuickActions, useContextAwareUI } from './ContextAwareUI';
import { 
  TrendingUp, 
  Clock, 
  Users, 
...
export const HybridSuperiorDashboard: React.FC = () => {
  ...
  return (
    <div className="flex-1 space-y-6 p-4 md:p-8">
      ...
      <div className="flex items-center justify-between bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl p-4 border border-orange-200 dark:border-orange-800">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold playwrite-font bg-gradient-to-r from-orange-500 to-pink-500 bg-clip-text text-transparent">
                OrderFi AI Dashboard
              </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button 
            ...
            className={`${isVoiceActive ? 'bg-orange-100 border-orange-300' : ''} flex items-center gap-2`}
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ${metrics.revenue.target.toLocaleString()} target
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                  ...
                  {metrics.orders.avgTime}min avg
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-orange-200 dark:border-orange-800">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-sm text-muted-foreground">
                  {metrics.customers.returning}% returning
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <Card className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-md border-orange-200 dark:border-orange-800">
            ...
            <CardContent>
              <div className="space-y-4">
                {[
                  { id: 1, table: 'Table 5', items: '2 Burgers, 1 Fries', time: '12 minutes ago', status: 'Cooking', color: 'orange' },
                  { id: 2, table: 'Table 2', items: '1 Salad, 2 Drinks', time: '8 minutes ago', status: 'Ready', color: 'green' },
                  { id: 3, table: 'Table 8', items: '3 Pizzas, 4 Drinks', time: '15 minutes ago', status: 'Preparing', color: 'blue' }
                ].map((order) => (
                  <div key={order.id} className={`flex items-center justify-between p-4 bg-${order.color}-50 dark:bg-${order.color}-900/20 rounded-lg border border-${order.color}-200 dark:border-${order.color}-800`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 bg-${order.color}-500 rounded-full flex items-center justify-center text-white font-bold`}>
                        #{order.id}
                      </div>
                      <div>
                        <p className="font-medium">{order.table} - {order.items}</p>
                        <p className="text-sm text-muted-foreground">Ordered {order.time}</p>
                      </div>
                    </div>
                    <Badge variant="outline" className={`bg-${order.color}-50 text-${order.color}-700 border-${order.color}-200`}>
                      {order.status}
                    </Badge>
                  </div>
                ))}
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-blue-600">12.3min</p>
...
export default HybridSuperiorDashboard;