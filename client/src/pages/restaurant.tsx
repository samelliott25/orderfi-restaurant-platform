...
export default function RestaurantPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"usdc" | "card" | "cash">(
    "usdc",
  );
  ...
  const calculateTotal = () => {
    ...
    const processingFee =
      paymentMethod === "usdc" ? subtotal * 0.0001 : subtotal * 0.03; // 0.01% vs 3%
    const mimiRewards =
      paymentMethod === "usdc" ? subtotal * 0.02 : subtotal * 0.01; // 2% vs 1%
        ...
        <Card className="bg-white shadow-modern border-modern max-w-md w-full mx-4">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-[#8b795e]/70 mb-4">
              Your order has been sent to the kitchen and payment processed
              automatically.
            </p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-modern-lg border border-[#8b795e]/10 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[#8b795e]/70">
                Web3-native dining experience • Instant USDC payments • Zero
                fees
              </p>
            </div>
            ...
            <Tabs defaultValue="popular" className="w-full">
              ...
              <TabsContent value="popular" className="space-y-4 mt-6">
                {menuItems.slice(0, 6).map((item: any) => (
                  <Card
                    key={item.id}
                    className="bg-white shadow-modern hover-lift border-modern"
                  >
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-semibold text-[#8b795e]">
                              {item.name}
                            </h3>
                          ...
                          <p className="text-sm text-[#8b795e]/70 mb-2">
                            {item.description}
                          </p>
                          <div className="flex items-center gap-4">
                  ...
                  .map((item: any) => (
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <h3 className="font-semibold text-[#8b795e] mb-1">
                              {item.name}
                            </h3>
                            <p className="text-sm text-[#8b795e]/70 mb-2">
                              {item.description}
              ...
              <TabsContent value="drinks" className="space-y-4 mt-6">
                {menuItems
                  .filter((item: any) => item.category === "Beverage")
                  .map((item: any) => (
                    <Card
                      key={item.id}
                      className="bg-white shadow-modern hover-lift border-modern"
              ...
              <TabsContent value="desserts" className="space-y-4 mt-6">
                {menuItems
                  .filter((item: any) => item.category === "Dessert")
                  .map((item: any) => (
                    <Card
                      key={item.id}
                      className="bg-white shadow-modern hover-lift border-modern"
                ...
                {cart.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {cart.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between p-2 rounded-lg border border-[#8b795e]/10"
                      >
                        <div className="flex-1">
                          <p className="font-medium text-[#8b795e] text-sm">
                            {item.name}
                          </p>
                ...
                <CardContent className="space-y-4">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-[#8b795e]">
                              USDC (Recommended)
                            </p>
                            <p className="text-xs text-green-600">
                              0.01% fee • 2x MIMI rewards
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="border-t border-[#8b795e]/10 pt-4 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[#8b795e]">
                        ${calculateTotal().processingFee}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-green-600">MIMI Rewards</span>
                      <span className="text-green-600">
                        +{calculateTotal().mimiRewards}
                      </span>
                    </div>
